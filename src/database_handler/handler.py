...
class DatabaseHandler:
    ...
    def __init__(self, db_path: Path = Path('/data/database/stocks.db')):
    ...
    def update_stock_data(self, tic: str, update_values: dict):
        update_data(self.db.get_connection(), "stocks", update_values, {"tic": tic})
    ...
    def fetch_stock_info(self, tic: Optional[int] = None) -> pd.DataFrame | tuple:
        if tic:
            rec = fetch_record(self.db.get_connection(), "stocks", {"tic": tic})
            return rec
        else:
            df = fetch_data(self.db.get_connection(), "stocks")
            df['last_update'] = pd.to_datetime(df['last_update'], format='ISO8601').dt.date
            return df
    ...
    def fetch_financial_data(self, tic: str) -> pd.DataFrame:
        ...
        df['rdq'] = pd.to_datetime(df['rdq'], format='ISO8601')
        return df
    ...
    def fetch_insider_data(self, tic: str) -> pd.DataFrame:
        ...
        df['filling_date'] = pd.to_datetime(df['filling_date'], format='ISO8601')
        return df
