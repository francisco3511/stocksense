...
def insert_data(connection: Connection, table_name: str, data: pd.DataFrame) -> None:
    try:
        columns = ', '.join(data.columns)
        placeholders = ', '.join(['?'] * len(data.columns))
        sql = f'INSERT OR REPLACE INTO {table_name} ({columns}) VALUES ({placeholders})'
        cursor = connection.cursor()
        cursor.executemany(sql, data.to_records(index=False))
        connection.commit()
    except Error as e:
...
def update_data(connection: sqlite3.Connection, table_name: str, update_values: dict, condition: dict) -> None:
    try:
        cursor = connection.cursor()
...
def count_data(connection: Connection, table_name: str, column: str) -> Optional[int]:
    try:
        ...
        logger.error(f"Error counting distinct stocks: {e}")
        return None


def fetch_record(connection: Connection, table_name: str, condition: Optional[dict] = None) -> Optional[pd.DataFrame]:
    try:
        cursor = connection.cursor()
        if condition:
            where_clause = ' AND '.join([f"{col} = ?" for col in condition.keys()])
...
def fetch_data(connection: Connection, table_name: str, condition: Optional[dict] = None) -> Optional[pd.DataFrame]:
    try:
        cursor = connection.cursor()
        if condition:
            where_clause = ' AND '.join([f"{col} = ?" for col in condition.keys()])
